...
/*   Updated: 2020/07/18 11:59:35 by mpivet-p         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
			...
			, t_vecint *draw_start, double *z_buffer, int8_t tex_id)
{
	...
	(void)color;
	x = draw_start->x;
	while (x < draw_end->x && x < SIMG_X)
	{
		tex.x = (256 * (x  - (-1 * sprite_size / 2 + sprite_screen_x)) * TEX_WIDTH / sprite_size) / 256;
...
static void	prepare_sprites(t_core *wolf, t_sprite *curr_sprite, double correc, double *z_buffer)
{
	...
	transform.y = correc * (wolf->cam.plane.x * sprite.y - wolf->cam.plane.y * sprite.x);
